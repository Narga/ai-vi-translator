...
- 🤖 **Multi-Provider AI & Resolver**: Hỗ trợ Google Gemini và OpenAI-compatible API (OpenRouter, Cloudflare AI Gateway, Stepfun, Mistral, proxy). Phân giải và validate model qua `ProviderConfigResolver`, bảo vệ chống cấu hình sai namespace model, tự động che giấu (mask) API key trên UI.
...
- 🧪 **Test Suite**: Unit + integration acceptance tests cho backend, CLI, WebUI và helpers (421 unit tests + 11 real Flask integration tests).
