...
#
# Nâng cấp v2.8.2:
# - Loại bỏ hoàn toàn khởi tạo file_manager (GeminiProjectFileManager đã xóa).
# - Vá logic verification mode: so sánh danh sách file input với output/parts.
# - Chuyển progress_dir sang cache_dir / "progress" để bảo toàn khi xóa cache chính.
...
# Tên file lưu trạng thái dịch thuật (cho file đơn chia chunk)
...
def run_translation_workflow(config_parser, dirs: Dict[str, Path]) -> None:
    ...
    config_params = {
        ...
        'input_lang': config_parser.get('PROCESSING', 'INPUT_LANG', fallback='CN'),
        'correction_mode': config_parser.get('PROCESSING', 'CORRECTION_MODE', fallback='parallel'),
    }
    ...
    if output_dir.exists() and (output_dir / 'parts').exists():
        ...
        if verify_choice == 'y':
            ...
            # Chạy verification (so sánh input với output/parts)
            verify_existing_translation(
    ...
    # progress_dir nằm trong cache để bảo toàn khi xóa cache chính (.pkl)
    ...
    if target_path.is_dir():
        ...
        translate_directory_project(
            ...
            prompts, config_params, normalizer, statistics, base_filename
        )
        ...
        translate_single_file_project(
            ...
            prompts, config_params, normalizer, statistics, health_monitor, base_filename
        )
...
def translate_directory_project(
    ...
    4. Ghép nối thành <base_filename>_full.txt
    ...
    Args:
        ...
        base_filename (str): Tên dự án (dùng cho file output)
    """
    ...
    # Ghép nối file <base_filename>_full.txt
...
def translate_single_file_project(
    ...
    5. Ghép nối thành <base_filename>_full.txt
    
    Args:
        source_file (Path): File nguồn
        output_dir (Path): Thư mục output
        progress_dir (Path): Thư mục tạm (cache/progress)
        api_manager: Trình quản lý API
        cache_manager: Trình quản lý cache
        prompts (Dict): Dictionary prompt
        config_params (Dict): Tham số cấu hình
