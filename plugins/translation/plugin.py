...
class Plugin(ProcessorPlugin):
    ...
    def initialize(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.translation_config = {
                "model_name": self.config_service.get(
                    "MODEL", "MODEL", fallback="gemini-2.5-flash"
                ),
                "qa_model": self.config_service.get(
                    "MODEL", "QA_MODEL", fallback="gemini-2.5-flash"
                ),
                "temperature": self.config_service.get(
                    "PROCESSING", "TEMPERATURE", fallback=0.75, value_type=float
                ),
                "max_refinement_attempts": self.config_service.get(
                    "PROCESSING", "MAX_REFINEMENT_ATTEMPTS", fallback=2, value_type=int
                ),
                "min_length_ratio": self.config_service.get(
                    "PROCESSING", "MIN_LENGTH_RATIO", fallback=0.5, value_type=float
                ),
                "max_length_ratio": self.config_service.get(
                    "PROCESSING", "MAX_LENGTH_RATIO", fallback=5.0, value_type=float
                ),
                "input_lang": self.config_service.get(
                    "PROCESSING", "INPUT_LANG", fallback="CN"
                ),
                "correction_mode": self.config_service.get(
                    "PROCESSING", "CORRECTION_MODE", fallback="parallel"
                ),
                "context_char_count": self.config_service.get(
                    "PROCESSING", "CONTEXT_CHAR_COUNT", fallback=500, value_type=int
                ),
            }

            # Chunking config
            self.min_chunk_size = self.config_service.get(
                "PROCESSING", "MIN_CHARS_PER_CHUNK", fallback=18000, value_type=int
            )
            self.max_chunk_size = self.config_service.get(
                "PROCESSING", "MAX_CHARS_PER_CHUNK", fallback=22000, value_type=int
            )

            self.logger.info(f"✓ {self.display_name} initialized")
            self.logger.info(f"  Model: {self.translation_config['model_name']}")
            self.logger.info(
                f"  Chunk size: {self.min_chunk_size}-{self.max_chunk_size} chars"
            )
            ...
            self.logger.error(
                f"Failed to initialize translation plugin: {e}", exc_info=True
            )
            return False
    ...
    def chunk_text(self, text: str) -> list[str]:
        ...
        Returns:
            List of text chunks
        """
        return process_text_for_chunking(text, self.min_chunk_size, self.max_chunk_size)
